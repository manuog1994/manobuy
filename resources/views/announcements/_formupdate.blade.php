<form action="{{route('announcements.update', $announcement->id)}}" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-3">
        ...
        <select class="form-select form-select-lg mb-3" aria-label=".form-select-lg example" name="category_id"
            ...
            <option value="{{$announcement->category->id}}" {{$announcement->category->id ? 'selected' : ''}}>
                {{__("ui.{$announcement->category->name}")}}</option>
        </select>
        @error('option')
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" id="form1" aria-describedby="nameHelp" placeholder=""
            data-mdb-showcounter="true" maxlength="60" name="name" value="{{$announcement->name}}">
    ...
    <div class="mb-3">
        ...
        <div class="input-group">
            ...
            <input type="number" class="form-control" id="exampleInputPrice" name="price" value="{{$announcement->price}}">
        </div>
    ...
    <div class="mb-3">
        ...
        <textarea class="form-control" id="exampleFormControlTextarea1" rows="10" name="description"
            aria-describedby="descriptionHelp" minlength="50">{{$announcement->description}}</textarea>
    ...
    <div class="d-flex justify-content-center mb-5">
        <button id="send" type="submit" class="btn btn-info btn-rounded mt-3">{{__('ui.modif')}}</button>
    </div>
</form>
